...
def test_should_raise_error_when_invalid_balance_cents_provided():
    with pytest.raises(InvalidAccountError) as e:
        Account.build(id, iban=iban, bic=bic, organization_name='name', balance_cents='invalid', created_at=created_at)
    assert e.value.message == 'Field balance_cents must be a valid float number'
...
def test_should_raise_error_when_invalid_organization_name():
    with pytest.raises(InvalidAccountError) as e:
        Account.build(id, iban=iban, bic=bic, organization_name=2, balance_cents=100.00, created_at=created_at)
    assert e.value.message == 'Field organization_name must be a valid string'


def test_should_build_account_correctly():
    account = Account.build(id, iban=iban, bic=bic, organization_name='name', balance_cents=100.00,
                            created_at=created_at)

    assert account == Account.build(id, iban=iban, bic=bic, organization_name='name', balance_cents=100.00,
                                    created_at=created_at)
...
def test_should_raise_error_when_insufficient_funds():
    with pytest.raises(InsufficientFundsError) as e:
        ...
        account.do_transfer(counterparty_name='name', counterparty_iban=Iban(iban),
                            counterparty_bic=Bic(bic), quantity=10.00, description='description')
...
def test_should_raise_error_when_doing_transfer_to_same_account():
    with pytest.raises(InvalidTransferOperationError) as e:
        account = Account.build(id, iban=iban, bic=bic, organization_name='name', balance_cents=20.00,
                                created_at=created_at)
        account.do_transfer(counterparty_name='name', counterparty_iban=Iban(iban),
                            counterparty_bic=Bic(bic), quantity=10.00, description='description')
...
def test_should_do_transfer_and_add_event():
    ...
    account.do_transfer(counterparty_name='name', counterparty_iban=Iban(another_iban),
                        counterparty_bic=Bic(another_bic), quantity=10.00, description='description')

    assert account.balance_cents == 10.00
    ...
    assert transfer_done_event['data']['transfer']['amount_cents'] == 10.00
